...
@section('content')
    <form class="grid-form" action="{{ route('admin.members.update',array($user->id)) }}" method="post">
        ...
        <fieldset>
            ...
            <div data-row-span="2">
                <div data-field-span="1">
                    ...
                    <input type="text" name="application_id" required value="{{(!is_null($user->application_id)) ? $user->application_id : 'User has not submitted an application.'}}">
                </div>
                <div data-field-span="1">
                    <label>Virtual Personnel File ID</label>
                    <input type="text" name="vpf_id" required value="{{(!is_null($user->vpf_id)) ? $user->vpf_id : 'User has not been provisioned a Virtual Personnel File.'}}">
                </div>
            </div>
            <div data-row-span="4">
