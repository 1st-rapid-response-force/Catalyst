...
@section('content')
    ...
    <div class="box">
        ...
        <div class="box-body">
            ...
            <p><button type="button" class="btn btn-success" data-toggle="modal" data-target="#newAwards">New Awards</button></p>
            <hr>
            <br>
            @if (count($awards) != 0)
                <table id="awards" class="table table-bordered table-hover">
                    <thead>
                    <tr>
                        <th>Image</th>
                        <th>Name</th>
                    ...
                    @foreach($awards as $award)
                        <tr>
                            <td><img src="{{$award->public_image}}"></td>
                            <td>{{$award->name}}</td>
                            <td>{{$award->description}}</td>
                            @if (\Setting::get('catalyst.enablePromotionPoints') == 'true')
                                <td>{{$award->promotionPoints}}</td>
                            @endif
                            <td>
            ...
            @else
                <p>There is no Awards in the database, add one by using the Administrator tools.</p>
            @endif
        </div><!-- /.box-body -->
    </div><!-- /.box -->
@endsection
...
@section('modal')
            <!-- Modal -->
    <div class="modal fade" id="newAwards" tabindex="-1" role="dialog" aria-labelledby="newAwardsModal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="newAwardsModal">New Awards</h4>
                </div>
                <div class="modal-body">
                    <form class="form-horizontal" method="POST" enctype="multipart/form-data">
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" id="name" name="name" placeholder="Name of Award">
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        @if (\Setting::get('catalyst.enablePromotionPoints') == 'true')
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" class="form-control" id="promotionPoints" name="promotionPoints" placeholder="Promotion Points Awarded for Award">
                                </div>
                            </div>
                        @endif
                        <div class="form-group">
